...
export const StaffDrawer = ({ visible, onClose, record }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: values => {
            ...
            if (record.id) {
                api.patch(`/staff/${record.id}`, { ...values, phone: `+232${values.phone}` })
                    .then(res => res.data)
                    .then(({ message }) => {
                        ShowMessage('success', 'New Staff', message);
                        handleClose(true);
                    })
                    .catch(handle401Error)
                    .finally(() => setLoading(false))
            } else {
                api.post("/staff", { ...values, phone: `+232${values.phone}` })
                    .then(res => res.data)
                    .then(({ message }) => {
                        ShowMessage('success', 'New Staff', message);
                        handleClose(true);
    ...
    useEffect(() => {
        if (record) {
            const { id, givenNames, familyName, gender, phone, email, address } = record;

            formik.setFieldValue("givenNames", givenNames);
            formik.setFieldValue("address", address);
            formik.setFieldValue("email", email);
            formik.setFieldValue("gender", gender);
            formik.setFieldValue("phone", phone.replace("+232", ""));
    ...
    return (
        ...
        <Drawer width={size(width)} title={`${record ? 'Edit' : 'New'} Staff`} placement="right" onClose={() => handleClose()} visible={visible}>
            <div >
